...
test("courtyard overlap handles circles, rotated rectangles, and tangency", () => {
  // circle intersection distinguishes overlap, separation, and tangency
  ...
  // circle inside rectangle intersects even far from the edges
  expectSymmetric(circle(10, -7, 0.2), rectangle(10, -7, 4, 4, 45), true)

  // 45 degree rectangle rejects a circle in its empty AABB corner
  expectSymmetric(circle(0, 0, 0.2), rectangle(1, -1, 4, 0.2, 45), false)

  // 45 degree rectangle detects true overlap
  expectSymmetric(circle(0, 0, 0.2), rectangle(0.5, 0.5, 4, 0.2, 45), true)

  // inverse rotation uses the rectangle center and CCW direction
  ...
  // bounds-only shapes retain conservative fallback and strict tangency
